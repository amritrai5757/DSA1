...
int f(int i,int j,vector<int> &arr, vector<vector<int>>&dp){
    ...
    int mini=INT_MAX;
    for(int k=i;k<j;k++){
        int temp=arr[i-1]*arr[k]*arr[j] +f(i,k,arr,dp) + f(k+1,j,arr,dp);// operaton assuming left subarray solved & becomes 1 sized & right solved + calling left and right subaarray
